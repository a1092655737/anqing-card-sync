...
import { useState, useRef, useEffect, memo, useCallback, useMemo, type ChangeEvent } from 'react';
...
const POSITION_STORAGE_KEY = 'anqing_position_process';
const POSITION_HEADERS_KEY = 'anqing_position_headers';

function loadSavedTasks(): PositionTask[] | null {
  try {
...
function loadSavedHeaders(): string[] | null {
  try {
    const saved = localStorage.getItem(POSITION_HEADERS_KEY);
    if (saved) return JSON.parse(saved);
  } catch { /* ignore */ }
  return null;
}

function saveTasksToStorage(tasks: PositionTask[]) {
  try {
    localStorage.setItem(POSITION_STORAGE_KEY, JSON.stringify(tasks));
  } catch { /* ignore */ }
}

function saveHeadersToStorage(headers: string[]) {
  try {
    localStorage.setItem(POSITION_HEADERS_KEY, JSON.stringify(headers));
  } catch { /* ignore */ }
}
...
// Strip operator prefix from product name (e.g. "广电升卿1.2" -> "升卿1.2")
function stripOperatorPrefix(name: string): string {
  for (const cat of OPERATOR_CATEGORIES) {
    if (name.startsWith(cat.key)) {
      return name.slice(cat.key.length);
    }
  }
  return name;
}
...
// ===== Card Product Dropdown Cell (stacking mode) =====
interface CardProductCellProps {
  value: string;
  onChange: (v: string) => void;
  placeholder: string;
}

const CardProductCell = memo(function CardProductCell({ value, onChange, placeholder }: CardProductCellProps) {
  const { state } = useData();
  ...
  // Group products by operator, strip prefix for display
  const groupMap = useMemo(() => {
    ...
    state.products.forEach(p => {
      ...
      if (cat) {
        ...
        if (!result[cat.key].some(x => x.suffix === suffix)) {
          result[cat.key].push({ full: p.product_name, suffix });
        }
      }
    });
    return result;
  ...
  // Append suffix to value (stacking mode)
  ...
  // Parse current lines to check which are selected
  const selectedLines = value ? value.split('\n').filter(l => l.trim()) : [];

  return (
    <div ref={containerRef} className="relative">
      {/* Editable textarea + dropdown trigger button */}
      <div className="flex items-stretch bg-white rounded" style={{ minHeight: 30 }}>
        ...
        <button
          ...
          className="px-1 flex items-center justify-center rounded-r hover:bg-black/5 transition-colors flex-shrink-0"
          style={{ minHeight: 30 }}
        >
          ...
          {OPERATOR_CATEGORIES.map(cat => {
            ...
            return (
              <div key={cat.key}>
                ...
                {/* Product list: suffix names */}
                {products.length === 0 ? (
                  <div className="px-3 py-1 text-[10px] text-white/20">暂无</div>
                ) : (
...
const TextCell = memo(function TextCell({ value, onChange, placeholder, headerStyle }: TextCellProps) {
  ...
  return (
    <div className="flex items-stretch">
      <textarea
        ...
        style={{
          fontFamily: headerStyle ? '"NotoSansCJK-Bold", sans-serif' : 'inherit',
          fontWeight: headerStyle ? 700 : 400,
          fontSize: headerStyle ? 14 : 12,
          padding: '6px 8px',
          lineHeight: '1.5',
          textAlign: 'center',
          background: headerStyle ? 'rgba(167,139,250,0.06)' : '#fff',
          color: headerStyle ? '#a78bfa' : '#000',
          border: headerStyle ? '1px solid rgba(167,139,250,0.2)' : 'none',
          minHeight: headerStyle ? 40 : 30,
        }}
        placeholder={placeholder}
      />
    </div>
...
export default function PositionProcess() {
  const { state } = useData();
  const [tasks, setTasks] = useState<PositionTask[]>(() => loadSavedTasks() || mockTasks);
  const [headers, setHeaders] = useState<string[]>(() => loadSavedHeaders() || DEFAULT_HEADERS);
  ...
  useEffect(() => {
    ...
    setTasks(prev => {
      ...
      const newTasks = lockedTopics
        .filter(name => !existingNames.has(name))
        .map(name => ({ ...emptyTask(), topicName: name }));
  ...
  const updateHeader = (i: number, v: string) =>
    setHeaders(p => p.map((h, idx) => idx === i ? v : h));
    ...
    reader.onload = () => {
      try {
        ...
        if (data.headers && Array.isArray(data.headers)) {
          ...
          saveHeadersToStorage(data.headers);
        }
      } catch { /* ignore */ }
    };
  ...
  // Auto-save: persist to localStorage whenever tasks or headers change
  useEffect(() => { saveTasksToStorage(tasks); }, [tasks]);
  useEffect(() => { saveHeadersToStorage(headers); }, [headers]);
    ...
    <main className="max-w-[1920px] mx-auto px-6 py-6">
      ...
      <div className="rounded-lg" style={{ background: 'rgba(255,255,255,0.02)', padding: 12 }}>
        <div style={{ display: 'grid', gridTemplateColumns: GRID_COLS, gap: GAP }}>
          {headers.map((h, i) => (
            <TextCell key={'h'+i} value={h} onChange={(v) => updateHeader(i, v)} placeholder="" headerStyle />
          ))}
        </div>
